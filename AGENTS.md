...
- 后续版本分两步实施：`docs/changes/luowang-code-understanding/` 对应 v0.6.0 的内置代码深读方法，`docs/changes/luowang-multi-project/` 对应 v0.6.1 的同一操作者多项目管理。两份 Spec/Plan 已编写、功能尚未实现；v0.6.0 保持单项目并独立发布，v0.6.1 在其发布后启动。实现这些主题时读取对应 intent/spec/plan；新增内置方法仍沿用受控角色资源加载，不开启 Pi Skill 自动发现。
