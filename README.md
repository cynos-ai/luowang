...
第九轮后的无凭据容器诊断确认：`api.deepseek.com` 的 DNS 和 TCP 443 正常，TLS 因 `SELF_SIGNED_CERT_IN_CHAIN` 被 Node 拒绝；容器没有代理、额外 CA 或关闭证书校验的配置。评估驱动现将 DNS、TLS、超时和普通连接失败分开记录为固定类别，不保存错误消息、证书或请求内容。定向 19 项测试及完整本地验收通过；没有关闭 TLS 校验，也没有自动信任未知 CA。当前网络信任链修复前不应再申请模型重试预算。
