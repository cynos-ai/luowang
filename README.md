...
截图需求已调整为保持取证现场：禁止为截图清空/覆盖表单或隐去待验证内容。新 [截图采集完整性计划](docs/changes/luowang-screenshot-capture-integrity/plan.md) 接管旧截图拒绝方案，复用现有文字脱敏，计划改为正常采集加检测标签；保留自动归档、人工事后审核，不新增发布门禁。角色规则已更新，MCP 拒绝补丁与标签链路尚待实现，旧样本及失败结论保留。
