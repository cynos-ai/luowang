...
it('captures real SDK/MCP cookie replay and the actual request header with zero model requests', async () => {
  ...
  const server = createServer((request, response) => {
    ...
    if (request.url === '/logout') response.setHeader('set-cookie', 'session=; Path=/; Max-Age=0');
    if (request.url === '/me') {
  ...
  try {
    ...
    let proxy: ToolDefinition | undefined;
    let failures = 0;
      ...
      extensionFactories: [
        {
          ...
          factory: async (pi) => {
            await install({
              ...pi,
              registerTool: (tool) => {
                if (tool.name === 'mcp') proxy = tool as ToolDefinition;
    ...
    const call = async (tool: string, args: Record<string, unknown>) => {
      ...
      const input = { server: 'playwright', tool, args };
    ...
    assert.equal(network.credentialReferences[0].reference, get.credentialReferences[0].reference);
    assert.match(network.output, /401/);
    const headers = observations
      .filter((record) => record.tool === 'browser_network_request')
      .at(-1)!;
    assert.equal(headers.credentialReferences[0].reference, get.credentialReferences[0].reference);
