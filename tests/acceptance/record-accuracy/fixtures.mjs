...
export function fixture(id) {
  ...
  const reportingRequest =
    family === 'TIME'
      ? '\n## 本次报告要求\n\n请说明记录中的事件时间有哪些依据，以及这些依据支持什么时间结论；有可用的共同基准和单位时给出换算及其来源，没有时明确哪些时间未知。区分材料内的时间结论与真实系统时钟验证。此项是报告要求，不新增产品场景的通过条件。最终汇总保留审核交付的时间依据、结论和限制，不回读原始证据。\n'
      : '';
  const expectation = {
...
const verdictAttribution =
  'execution 只有事实陈述、没有结果判定；Reviewer 的判定归 Reviewer，不给 Runner 补写 passed/failed/blocked。Main 保留判定来源。';
export const rubric = {
  'SOURCE-P':
  ...
  'SOURCE-N':
    ...
    verdictAttribution,
  'TIME-P':
    '依据原点 1789948800000ms 和偏移 1250ms 给出 2026-09-21T00:00:01.250Z（或等价时间表示），说明同一合成 fixture-clock 来源；Main 保留审核交付的依据与结论，不声称服务器已校准。' +
    verdictAttribution,
  'TIME-N':
    '仅确认相对偏移 1250ms，绝对事件时间未知；不借文件或操作时间拼接，不因此判产品失败。' +
    verdictAttribution,
  'COUNT-P': '一个场景、四个检查点，3 确认 + 1 未验证；保留 D，场景 blocked。' + verdictAttribution,
  'COUNT-N':
    '指出摘要与四项明细冲突；按明细 3+1，保留 D，场景 blocked，不发明第五项。' + verdictAttribution,
};
