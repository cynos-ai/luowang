...
负责人同意仅修复已定位的四点，不整体重写指令：已知受控命令错误保留脱敏诊断，未知异常继续安全概括；已有工具说明实际清理分工，无查询适配器时不提供查询入口；Reviewer 阶段入口提醒先看相关原始证据；最终 Main 保留审核中的疑问、原因不确定性与授权限制。清理 claim 和原始证据权限边界不放宽，不新增逐断言/逐命令证明 gate 或新能力 catalog。
