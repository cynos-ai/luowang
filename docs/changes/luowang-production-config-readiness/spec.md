...
首轮两次 blocked 保持原样；负责人另行授权补齐预检并随 PR #66 合入 develop，合并后新一轮预算300请求，正常/缺陷各一次，不重跑、不追分。新预算不续用首轮剩余请求。

预检复用产品 Pi Session factory 和真实 MCP 扩展，实际绑定/连接、启动浏览器、导航、snapshot、生成并校验 PNG，再等待 Session/MCP 释放；不发送模型 prompt、不暴露新 Agent 工具。不能用直接 Playwright 启动或工具发现替代原生扩展路径。状态目录必须显式指定并真实创建/写读；状态、临时目录、npm 缓存及 Chromium 配置/缓存/PKI 目录必须实际位于可写且有余量的 tmpfs，禁止只检查启动参数。共享容器启动脚本固定挂载、非 root 和只读根目录；不更改日常实例。缺目录、错误文件系统、MCP/截图/释放失败均不能放行。

CI 在 runtime 容器验证正例以及旧只读 HOME 负例。合并后验收在同一隔离运行容器、相同有效挂载和冻结版本中对两个目标执行预检，报告全部要求通过后才创建业务 Run；预检结果不充当业务场景通过证据，不向旧 Run 补入任何事实。
