...
- 状态：v0.6.0 已发布；多项目生产入口已接入完整离线升级门禁、项目 App、全局调度和控制台，真实旧实例尚未迁移或做双项目联合验收。阶段 2 的项目镜像与受控命令已接入，但 Docker 部署、重启回收和不同工具链的正式验收仍在推进。2026-09-24 按负责人确认加入项目专属可复用执行镜像，依赖与代码版本必须随固定目标提交核验。
