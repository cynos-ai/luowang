...
### 2026-09-26 最终五节点执行计划

负责人已授权连续推进、沿用测试资源及模型调用，不因例行 push 或检查等待重新确认。最终发布报告完成后交负责人审核，审核通过才合并发布。
...
迁移验收以 Spec 第 7、8 节为准：隔离样例可验证迁移与失败路径，没有必须取得生产旧库的附加发布条件。专用非生产应用可以设置明确、可逆的两个测试故障，并由真实执行和 Reviewer 确认；不手填结论、不把环境停机称为产品 Bug。测试服务器的部署形态验收已完成，其硬件配置不限制产品支持范围。
...
2026-09-26 最终候选修正：通用 live 门禁已按项目 API 的平铺 StoredRun 格式读取归档状态与工件映射。隔离复制真实 Closure 7 持久库后，离线升级成功，8 条 Run 的结果、固定提交、时间、报告路径、Bug 与证据关键内容摘要前后一致（SHA-256 `677868931faf3af5c27d908c53ca04922f691528dbce85ec7ab8591bb9ea7bda`），7 项 Secret 解密一致，外键检查通过；此摘要不包括归档状态，不能据此声称所有列不变。原库与升级备份均保留。
...
## v0.6.1 最终候选审核报告（2026-09-26）

发布前的代码、迁移和联合验收已完成，建议进入负责人审阅。当前仍是候选版：未合入 develop/main，未创建 v0.6.1 tag，也未发布。下面是本计划的最新状态；上方按日期记录的失败、限制和当时缺项继续保留。

### 本版交付

同一个管理员可接入、切换和管理多个 GitHub 项目。项目分别保存配置、凭据、场景、Run、证据和归档，允许不同仓库有相同场景 ID 或历史报告 ID。控制台把账号、部署和项目设置分开，新项目保存后保持暂停，管理员完成就绪检查再启用。

每个项目使用自己的可复用执行镜像，语言和依赖留在项目镜像中；Run 固定源码提交和镜像 digest。全局仍一次执行一个 Run，项目之间轮转；某个项目失败或等待归档时，其他项目可以继续。Docker 不可用时明确失败，不回退到服务容器执行项目命令。

最后一轮修复了迁移项目首次恢复前的后台副作用、旧工件归属读取，以及模型探测误报成功。模型 SDK 即使以正常 Promise 返回错误响应，就绪检查也会拒绝；测试使用真实 SDK 连接返回 503 的本地网关，并验证响应内容不泄露。

### 验收结果与证据

| 检查 | 最终结果 | 证据及范围 |
| --- | --- | --- |
| 工程质量 | 通过 | 干净 Linux quality：格式、lint、类型、构建、444 passed / 2 skipped，三组浏览器 E2E；两项默认跳过的 Docker 专项已有前文独立真实 Engine 验证 |
| 完整发布前验收 | local=passed、live=passed、release=passed | 本机受控 `final-results/release-1790400681229/report.json`；此处 release 表示发布前检查通过，不代表已发布 |
| 生产浏览器 | 通过 | 最终 runtime 在非 root、只读文件系统、受限 tmpfs、无网络条件下完成原生 MCP 导航、快照、截图与 Session 释放，模型请求为 0 |
| 三项目联合事实 | 通过 | 最终 runtime 复核 3 个项目、5 个代表 Run、20 张截图 SHA-256、13 项清理回执、各仓库正式报告与跨项目 404；保留原 passed/blocked 结论 |
| 真实旧实例迁移与恢复 | 通过 | 复制持久 Closure 7 测试实例，离线升级后 7 项 Secret 解密一致；首次恢复前无后台副作用，恢复后两条历史归档失败队列正常重试 |
| 历史内容保留 | 通过 | 8 条 Run 的关键事实、30 份工件正文、2 条 Issue 编号/URL/状态与备份相同，外键无错误；哈希见下方 |
| 备份回退 | 通过 | 校验备份清单后恢复到一次性目录，旧 schema 可识别、SQLite 完整性正常、8 个 target commit 均存在；未用旧程序打开迁移后的数据库 |
| 服务器部署形态 | 已通过 | 前述测试机的隔离 Compose、Docker 权限、实例资源回收及失联恢复；完整模型/浏览器负载在本机候选执行 |

最终生产镜像 ID 为 `sha256:7882855ab151a83b7ad9cca206e85acabbd0fe938a32d0e37656d42ff5f2b4e2`，包含版本 0.6.1 和代码修复 `3bceb5d`。本地镜像不是已发布的镜像地址。GitHub Quality 的最终结果以 [PR #79](https://github.com/cynos-ai/luowang/pull/79) 当前提交检查为准。

历史核对摘要：Run 关键事实 SHA-256 `a2e44b073a3873371f7311d1ccbddd4e4b99ce8716abbe845b4b95b336a708d2`；30 份工件正文 `4439d2b47a8599edddfb4efffd1b96fa0ad10a9802f3adac962ea509bbb9bd00`；2 条 Issue 事实 `b69b52a906d0da9b677c524db5eccc82809a173df03a03fc38fb779cafe0f66f`。这些摘要限定于脚本核对的内容，不声称所有数据库列都不变。

### 本轮新增的真实 Run

`01M3E20CPNZ88WHADECPSQEG34` 在固定 target `0defd30be3761c7ad8ba66bfd96d65737f54245b` 执行登录与注册场景，两项均 passed。四个独立 Session 分别完成规划、执行、审核和最终报告；Main/Runner 使用 `deepseek-v4-flash`，Reviewer 使用 `deepseek-v4-flash-vision-exp`。

本次有 147 项证据，其中 16 张截图已通过项目 Evidence Gateway 回读并校验字节数及 SHA-256；3 项 Harness 清理回执为 `absent=true`。正式报告已自动归档到 `b3392c6eee5e1dec6daa4cf7dd32b6c0fcb81c91`，远端正文与索引一致：[查看正式报告](https://github.com/cynos-ai/luowang-closure7-fixture/blob/b3392c6eee5e1dec6daa4cf7dd32b6c0fcb81c91/docs/scenario-testing/reports/01M3E20CPNZ88WHADECPSQEG34/report.md)。SDK catalog 估算费用为 USD `0.087509464`，不等于供应商账单。

这次模型 Run 使用 `748bc7c` 对应运行逻辑；随后只增加 Provider 就绪探测错误判断，并在最终 runtime 上重跑 readiness、浏览器预检、历史/新 Run 证据复核和完整发布前验收。没有把旧初始化、双 Bug/Issue、场景 PR 等历史流程写成这轮重新执行。通用 live 复核这些历史真实事实；独立多项目 live 复核本版已执行的交替任务和故障隔离。

### 保留的失败与边界

- 旧 localhost:3200 模型网关不存在，导致 `01M3E1TEVK3TEDHR7S57BHV6X8` 在规划前失败；记录保留。配置改为当前已授权的模型 endpoint 后，新 Run 完成。
- 新 Run 曾因原 GitHub 凭据不能写入而归档失败；通过项目 Secret Store 轮换为已授权凭据，自动重试后归档完成。没有重跑测试或改写结论。
- 第一个迁移候选的归档重试失败保留在原候选卷；修复在第二个隔离副本验证，未修改原始旧实例或备份。
- 第一次最终 release 检查为 local=passed、live=failed：新报告提交后项目镜像尚未就绪，期间 GitHub 连接也有失败。同步及镜像准备完成后，在最终 runtime 重跑得到三层 passed。两份检查记录都保留。开发机 Docker 代理故障使用同一引擎的本地 socket 绕过；Git 连接使用 HTTP/1.1，未关闭 TLS 校验或重启共享 Docker。
- 本轮没有重跑 v0.6.0 的八类、每类三次模型效果对比；聚合报告中相应 AC-SDQ 项仍为 not_run。本版保留代码深读工程回归和真实 Run，不据此宣称模型质量进一步提高。
- 仍是单管理员、全局串行执行，只接入可信仓库。项目容器用于隔离工具链和运行状态，不承诺隔离恶意代码，也不会自动部署被测应用。升级属于停机备份后的离线切换，不提供旧 API 双轨兼容。
- 测试服务器未执行完整模型/浏览器负载，其硬件余量不是罗网最低配置。实际部署须根据项目镜像、构建缓存、浏览器和其他同机服务测量容量。

### 审核后还有两个节点

1. 通过 PR #79 将功能分支合入 develop，再建立 develop → main 发布 PR；在 main 打 v0.6.1 annotated tag、发布说明，并核验 main/tag 指向。AC-MP-12 的发布后部分与 `AC-CLOSURE-RELEASE-01` 在此之前继续未完成。
2. 基于发布提交构建并复核正式 runtime，带实际发布 tag 重跑发布核验；按实例归属清理临时容器、网络和可回收镜像，保留原始历史、升级备份及全部失败证据。原三项目卷、两份迁移候选卷和备份当前保留供审阅与回退，不执行全局 prune。

无需负责人补充模型预算、测试账号或服务器资料。最终合并发布仍按既定计划等待负责人审阅本报告；这不是新增的报告归档门禁，普通 Run 继续自动归档、人工事后审核。
