...
- 状态：v0.6.0 已发布；v0.6.1 的多项目入口、离线升级、全局调度和项目镜像已实现。两个 Node 目标与异构 Python 目标已完成交替真实 Run，并做了单项目应用停机、其他项目继续完成的隔离对照；Python 全量 Run 的一项 blocked 已由后续定向复测闭合，旧失败/blocked 记录保留。负责人指定的测试服务器已完成隔离 Compose 部署形态与 Docker 回收验收；该机器未运行完整模型/浏览器负载，也不作为该负载的单独发布门禁，不能据此推断罗网支持的最低服务器配置。正式旧实例迁移条件、候选资源收尾和最终发布审核仍未完成，详见本计划末尾的 2026-09-26 验收环境说明。
