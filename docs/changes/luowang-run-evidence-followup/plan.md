...
- 已拉取新增 intent 的远端提交 5ed75d0。后续截图工作由 [screenshot-capture-integrity 计划](../luowang-screenshot-capture-integrity/plan.md) 接管；该目录 Spec 覆盖本计划原“非空字段拒绝、清空后重拍”的目标。禁止为了截图修改现场，从角色规则开始执行。
- 本 change 保留 Cookie 关联、文字脱敏、内联快照登记、读取完整性与已经完成的验证。新的填写值登记、截图警告/标签及状态完整性验收在新目录实施，不建立重复 owner。
- 原稳定/状态依赖截图案例不再按清空方案继续。下面的旧步骤、案例设计和完成记录是当时事实；原 fail、原图及执行文本不改写，不因新规格倒改历史结论。
- 同分支继续 PR #71，不先合并旧方案。报告保留自动归档、人工事后审核，不新增发布门禁。截图拒绝补丁已替换为正常采集及哈希绑定标签，证据工具、控制台和最终报告已接入；无旧格式兼容。接下来补填写登记及联合回归，完整证明在新计划维护。live/release 仍 blocked，人工评分仍 not_run。
