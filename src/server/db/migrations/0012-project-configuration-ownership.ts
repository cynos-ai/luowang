...
export function migrateLegacyConfigurationOwnership(
  ...
  if (!project) throw new Error('旧配置迁移的项目不存在');
  ...
  if (!repositoryRow) throw new Error('旧仓库配置不存在');
  const repository = parseStoredObject(repositoryRow.value, '旧仓库配置');
  if (typeof repository.repository !== 'string') throw new Error('旧仓库配置缺少仓库身份');
  const configuredRepository = parseGitHubRepository(repository.repository);
  if (
